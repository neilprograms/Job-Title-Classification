...
# job level labels
job_role = ["C-level", "Director", "Individual contributor", "Manager", "Vice President"]
...
X, y = underSampler.fit_resample(X, trainTestDf['Role'])
...
sns.heatmap(cm, annot=True, xticklabels=job_role, yticklabels=job_role) 
plt.show()
...
print("Accuracy:", dict(zip(job_role, accuracy.tolist())))
print("Precision:", dict(zip(job_role, precision.tolist())))
print("Recall:", dict(zip(job_role, recall.tolist())))
print("F1 Score:", dict(zip(job_role, f1_score.tolist())))
...
def predict(title):
    ...
    padded = pad_sequences(seq, maxlen=10, dtype='int32', padding='post', truncating='post')
    pred = model.predict(padded)
    return job_role[np.argmax(pred)]
...
LSTMRolePreds = pd.DataFrame(preds, columns=['Title', 'Role'])

# turn into excel sheets
LSTMRolePreds.to_excel('LSTMRoleUnder.xlsx')
